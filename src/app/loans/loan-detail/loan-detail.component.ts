...
export class LoanDetailComponent implements OnInit, OnDestroy {
  ...
  constructor(private loansService: LoansService, private router: Router, private route:  ActivatedRoute,
              private toastr: ToastrService, private fb: FormBuilder, private credentialService: CredentialsService) { }
  ...
  getLoanDetails(id: any) {
    ...
    calculateRepaymentTotal(event: any) {
      ...
      // return principal + ((interest / 100) * principal);NG
        this.borrowerMsg = `${this.formatCurrency(principal)} to pay back ${this.formatCurrency(amountToPay)} in
    ...
    bankAccountLookup(accountNumber: any) {
        this.isLoading = true;
       const lookUp$ = this.loansService.validateAccountNumber(accountNumber,
        ...
        lookUp$.pipe(
            ...
            (res: any) => {
                if (res.responseCode === '00') {
                    ...
                    if ( this.accountName.includes(res.responseData.last_name) ||
                        this.accountName.includes(res.responseData.first_name)) {
                        this.isValidBvn = true;
                        this.userBvn = bvn;
                        this.toastr.success(`Name on BVN matches name on bank account`, undefined, {
                            closeButton: true,
                            positionClass: 'toast-top-right'
                        });
                     } else {
                        this.isValidBvn = false;
                        this.toastr.error(`Name on BVN completely different from
                        name on bank account, please check and re-apply`, undefined, {
                            closeButton: true,
                            positionClass: 'toast-top-right'
                        });
                    }
